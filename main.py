...
for contending_devices in range(0, plot_x_number):
    ...
    for t in range(runs):
        ...
        for i in range(number_of_different_total_slots):
            b_list[i] = b_list[i] * \
                        np.emath.power(path_loss_db[t][t / (number_of_different_total_slots / number_of_device_required)], -0.5)
    ...
    print("SDA done")
    connected_device_sequence_MWFMP[contending_devices], datetime_sequence_MWFMP[contending_devices] = MWFMP(runs, p_matrix, number_of_device_required, number_of_PRB, bandwidth_per_PRB, number_of_slots_per_PRB)
    print("MWFMP done")
    connected_device_sequence_MIP[contending_devices], datetime_sequence_MIP[contending_devices] = MIPBranchCutGurobi(100, L,  p_matrix_copy, g_matrix, number_of_edges, number_of_device_required, bandwidth_per_PRB, number_of_PRB, Xi, N_noise)
    print("MIP done")
