...
for target_datarate_per_device in [10, 30, 50, 70, 90, 110, 130]:
    ...
    for t in range(runs):
        ...
        for i in range(int(number_of_edges / (L * number_of_subcarriers_perPRB))):
            ...
            for j in range(int(L * number_of_subcarriers_perPRB)):
                g_matrix[t][i * L * number_of_subcarriers_perPRB + j] = b_list_X[i]
    ...
    connected_device_sequence_SDA[plot], datetime_sequence_SDA[plot] = Baseline_SDA(runs, L, number_of_PRB,
                                                                                    transmission_power_per_PRB,
                                                                                    p_matrix, number_of_slots_per_PRB,
                                                                                    number_of_device_required, Xi)
    print("SDA done")
    connected_device_sequence_MWFMP[plot], datetime_sequence_MWFMP[plot] = MWFMP(runs, p_matrix_MWFMP,
                                                                                 number_of_device_required,
                                                                                 number_of_PRB,
                                                                                 transmission_power_per_PRB,
                                                                                 number_of_slots_per_PRB, L, Xi)
    print("MWFMP done")
    print(connected_device_sequence_SDA)
    print(connected_device_sequence_MWFMP)
    connected_device_sequence_MIP[plot], datetime_sequence_MIP[plot] = MIPBranchCutGurobi(1, L, p_matrix_copy,
                                                                                          g_matrix, number_of_edges,
                                                                                          number_of_device_required,
                                                                                          bandwidth_per_PRB,
                                                                                          number_of_PRB, Xi, N_noise)
    print("MIP done")
    plot += 1
