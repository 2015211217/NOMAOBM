...
number_of_slots_total = number_of_slots_per_PRB * number_of_PRB * L
...
for number_of_PRB in range(1, 6):
    ...
    for t in range(runs):
        ...
        for i in range(int(number_of_edges /(L * number_of_subcarriers_perPRB))):
            b_list_X[i] = (np.power(b_list[i].real, 2) + np.power(b_list[i].imag, 2)) / path_loss_w[t][int(i / (int(number_of_edges /(L * number_of_subcarriers_perPRB)) / number_of_device_required))]
